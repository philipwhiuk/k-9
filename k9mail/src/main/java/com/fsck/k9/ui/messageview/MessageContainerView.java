    ...
    public void renderCalendarEvents(MessageViewInfo messageViewInfo) {
        if (messageViewInfo.iCalendarEvents != null) {
            for (ICalendarViewInfo iCalendar : messageViewInfo.iCalendarEvents) {
                for (int i = 0; i < iCalendar.iCalData.length; i++) {
                    ICalData iCalData = iCalendar.iCalData[i];
                    ICalendarView view;
                    switch (iCalData.getMethod().getValue()) {
                        case "PUBLISH":
                        ...
                        default:
                            Log.i(K9.LOG_TAG, "Unhandled iCalendar method type:"
                                    + iCalData.getMethod().getValue());
                            continue;
                    }
                    view.setCallback(iCalendarCallback);
                    view.setICalendar(iCalendar, i);
                    view.setShowSummary(subject == null || !subject.equals(iCalendar.iCalData[i].getSummary()));
