...
public class SleepService extends CoreService {
    ...
    public static void sleep(Context context, long sleepTime, TracingWakeLock wakeLock, long wakeLockTimeout) {
        ...
        if (K9.DEBUG) {
            Log.d(K9.LOG_TAG, "SleepService Preparing CountDownLatch with id = " + id +
        ...
        try {
            ...
            if (!countedDown) {
                if (K9.DEBUG) {
                    Log.d(K9.LOG_TAG, "SleepService latch timed out for id = " + id +
                            ", thread " + Thread.currentThread().getName());
                }
            }
        ...
        if (releaseDatum == null) {
            try {
                if (K9.DEBUG) {
                    Log.d(K9.LOG_TAG, "SleepService waiting for reacquireLatch for id = " + id +
                            ", thread " + Thread.currentThread().getName());
                }
                if (!sleepDatum.reacquireLatch.await(5000, TimeUnit.MILLISECONDS)) {
                    Log.w(K9.LOG_TAG, "SleepService reacquireLatch timed out for id = " + id +
                            ", thread " + Thread.currentThread().getName());
                } else if (K9.DEBUG) {
                    Log.d(K9.LOG_TAG, "SleepService reacquireLatch finished for id = " + id +
                            ", thread " + Thread.currentThread().getName());
                }
            } catch (InterruptedException ie) {
