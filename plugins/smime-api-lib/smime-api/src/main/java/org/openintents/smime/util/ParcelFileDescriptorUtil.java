...
public class ParcelFileDescriptorUtil {
    ...
    public static <T> DataSinkTransferThread<T> asyncPipeToDataSink(
        ...
        InputStream inputStream = new BufferedInputStream(new ParcelFileDescriptor.AutoCloseInputStream(output));
        ...
        public void run() {
            try {
                ...
                if (dataSource.isCancelled()) {
                    Log.d(SMimeApi.TAG, "Stopped writing because operation was cancelled.");
                } else if (isIOExceptionCausedByEPIPE(e)) {
                    Log.d(SMimeApi.TAG, "Stopped writing due to broken pipe (other end closed pipe?)");
                } else {
                    Log.e(SMimeApi.TAG, "IOException when writing to out", e);
                }
            } finally {
    ...
    static class DataSinkTransferThread<T> extends Thread {
        final SMimeDataSink<T> dataSink;
        final InputStream inputStream;
        T sinkResult;

        DataSinkTransferThread(SMimeDataSink<T> dataSink, InputStream inputStream) {
