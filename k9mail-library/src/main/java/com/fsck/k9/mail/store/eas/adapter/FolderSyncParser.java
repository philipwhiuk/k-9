...
public class FolderSyncParser extends Parser {
    ...
    private List<EasStore.EasFolder> folderList;

    private EasStore easStore;

    public FolderSyncParser(InputStream in, EasStore easStore, List<EasStore.EasFolder> folderList) throws IOException {
        super(in);
        ...
        while (nextTag(START_DOCUMENT) != END_DOCUMENT) {
            if (tag == Tags.FOLDER_STATUS) {
                ...
                if (status != Eas.FOLDER_STATUS_OK) {
                    Log.e(K9MailLib.LOG_TAG, "FolderSync failed: " + status);
                    if (status == Eas.FOLDER_STATUS_INVALID_KEY) {
                        easStore.setStoreSyncKey("0");
                        Log.e(K9MailLib.LOG_TAG, "Bad sync key; RESET and delete all folders");
                        // EASTODO
                        res = true;
                    } else {
                        // Other errors are at the server, so let's throw an error that will
                        // cause this sync to be retried at a later time
                        Log.e(K9MailLib.LOG_TAG, "Throwing IOException; will retry later");
    ...
    void userLog(String ...strings) {
        Log.i(K9MailLib.LOG_TAG, Arrays.toString(strings));
    }

    void userLog(String string, int num, String string2) {
        Log.i(K9MailLib.LOG_TAG, string + num + string2);
    }
}
