...
import com.fsck.k9.mail.store.eas.EasStore.EasFolder;
...
public abstract class AbstractSyncParser extends Parser {
    ...
    public AbstractSyncParser(InputStream in, EasFolder folder) throws IOException {
        super(in);
        mFolder = folder;
    }
    ...
    public boolean parse() throws IOException, MessagingException {
        ...
        while (nextTag(START_DOCUMENT) != END_DOCUMENT) {
            if (tag == Tags.SYNC_COLLECTION || tag == Tags.SYNC_COLLECTIONS) {
                ...
                status = getValueInt();
                if (status != 1) {
                    Log.e(K9MailLib.LOG_TAG, "Sync failed: " + status);
                    ...
                    if (status == 3) {
                        ...
                        Log.e(K9MailLib.LOG_TAG, "Bad sync key; RESET and delete data");
                        wipe();
