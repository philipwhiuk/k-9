...
public class TracingPowerManager {
    ...
    public class TracingWakeLock {
        ...
        public void acquire(long timeout) {
            ...
            if (K9MailLib.isDebug()) {
                Log.v(LOG_TAG,
                        "TracingWakeLock for tag " + tag + " / id " + id + " for " + timeout + " ms: acquired");
            }
            raiseNotification();
            if (startTime == null) {
        ...
        public void acquire() {
            ...
            if (K9MailLib.isDebug()) {
                Log.w(LOG_TAG,
                        "TracingWakeLock for tag " + tag + " / id " + id +
        ...
        public void release() {
            if (startTime != null) {
                ...
                if (K9MailLib.isDebug()) {
                    Log.v(LOG_TAG,
                            "TracingWakeLock for tag " + tag + " / id " + id +
                                    ": releasing after " + (endTime - startTime) + " ms, timeout = " + timeout + " ms");
                }
            } else {
                if (K9MailLib.isDebug()) {
                    Log.v(LOG_TAG,
                            "TracingWakeLock for tag " + tag + " / id " + id +
                                    ", timeout = " + timeout + " ms: releasing");
                }
            }
            cancelNotification();
            synchronized (wakeLock) {
                    ...
                    timerTask = new TimerTask() {
                        ...
                        public void run() {
                            if (startTime != null) {
                                Long endTime = System.currentTimeMillis();
                                Log.i(LOG_TAG,
                                        "TracingWakeLock for tag " + tag + " / id " + id + ": has been active for "
                                      + (endTime - startTime) + " ms, timeout = " + timeout + " ms");

                            } else {
                                Log.i(LOG_TAG, "TracingWakeLock for tag " + tag + " / id " + id +
                                        ": still active, timeout = " + timeout + " ms");
                            }
                        }
