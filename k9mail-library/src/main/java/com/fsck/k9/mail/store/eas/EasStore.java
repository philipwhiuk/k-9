...
public class EasStore extends RemoteStore {
     ...
     * </pre>
     */
    public static EasStoreServerSettings decodeUri(String uri) {
        String host;
        int port;
        ...
        } else if (scheme.startsWith("eas+ssl")) {
            connectionSecurity = ConnectionSecurity.SSL_TLS_REQUIRED;
        } else if (scheme.startsWith("eas+tls")) {
        ...
        //TODO: Extras
        return new EasStoreServerSettings(ServerSettings.Type.EAS,
                host, port, connectionSecurity, null, username, password, null, new HashMap<String, String>());
    }
    ...
    public EasStore(StoreConfig storeConfig, TrustedSocketFactory trustedSocketFactory) throws MessagingException {
        ...
        mPolicyKey = settings.getPolicyKey();
        mSyncKey = settings.getSyncKey();
    ...
    public void validateAccount() throws MessagingException {
        throw new MessagingException("Validation not yet implemented");
        //TODO: Reimplement
        /**
        try {
     ...
     */
    //TODO: Move Security Policy Key into Store URI
    private void setHeaders(HttpRequestBase method, boolean usePolicyKey) {
        ...
        if (usePolicyKey) {
            ...
            if (!TextUtils.isEmpty(mPolicyKey)) {
                key = mPolicyKey;
            }
    ...
    private void init() throws IOException, MessagingException {
        synchronized (mInitializationLock) {
            ...
            if (mDeviceId == null) {
                mDeviceId = "deviceId";
                //TODO: Get unique ID to identify the device and application.
                // mDeviceId = K9MailLib.app.getDeviceId();
    ...
    private List <? extends Folder > getInitialFolderList() throws MessagingException {
        ...
        for (EasFolder folder : folderList) {
    ...
    private void syncFoldersFromLocalStore() {
        ...
        try {
            ...
            if (localStore != null) {
                ...
                synchronized (mFolderList) {
                    for (EasFolder folder : localFolders) {
...
//                }
    ...
    public class EasFolder extends Folder {
        ...
        public void open(int mode) throws MessagingException {
            open(mode == Folder.OPEN_MODE_RO ? OpenMode.READ_ONLY : OpenMode.READ_WRITE);
        }

        //TODO: Make expected method
        public void open(OpenMode mode) throws MessagingException {
            mIsOpen = true;
        }

        //TODO: Implement proper copy method
        ...
        @Override
        public List<EasMessage> getMessages(int start, int end, Date earliestDate, MessageRetrievalListener listener)
        throws MessagingException {
            try {
                return getMessagesInternal(null, null, null, start, end);
            } catch (IOException e) {
            ...
            while (moreAvailable && easMessages.isEmpty()) {
                ...
                try {
                    ...
                    if (code == HttpStatus.SC_OK) {
                        InputStream is = resp.getEntity().getContent();
                        if (is != null) {
                            EasEmailSyncParser parser = new EasEmailSyncParser(is, this);
    ...
    public class EasPusher implements Pusher {
        ...
        public void start(final List<String> folderNames) {
            ...
            Runnable runner = new Runnable() {
                public void run() {
                    wakeLock.acquire(K9MailLib.PUSH_WAKE_LOCK_TIMEOUT);
            ...
            try {
                ...
                try {
                    ...
                    if (code == HttpStatus.SC_OK) {
                        InputStream is = resp.getEntity().getContent();
                        if (is != null) {
                            EasEmailSyncParser syncParser = new EasEmailSyncParser(is, folder);
                            syncParser.parse();
