...
import com.fsck.k9.mail.store.eas.EasEmailSyncParser;
import com.fsck.k9.mail.store.eas.FolderSyncParser;
import com.fsck.k9.mail.store.eas.PingParser;
import com.fsck.k9.mail.store.eas.ProvisionParser;
import com.fsck.k9.mail.store.eas.Serializer;
import com.fsck.k9.mail.store.eas.Tags;
import com.fsck.k9.mail.ssl.TrustedSocketFactory;

public class EasStore extends RemoteStore {
    ...
    public static ServerSettings decodeUri(String uri) {
        ...
        return new EasStoreSettings(host, port, connectionSecurity, null, username, password, null);
    }
    ...
    public EasStore(StoreConfig storeConfig) throws MessagingException {
    ...
    public String getStoreSyncKey() {
        String key = mStoreConfig.getSyncKey();
    ...
    public void setStoreSyncKey(String syncKey) {
        mStoreConfig.setSyncKey(syncKey);
        //TODO: mStoreConfig.saveSyncKey(Preferences.getPreferences(K9.app.getApplicationContext()));
    }
    ...
    public void validateAccount() throws MessagingException {
        try {
            Log.i(K9MailLib.LOG_TAG, "Testing EAS: " + mHost + ", " + mUsername + ", ssl = " + (mSecure ? "1" : "0"));

            EasStore svc = new EasStore(mStoreConfig);
            ...
            Log.e(K9MailLib.LOG_TAG, "Validation (OPTIONS) response: " + code);
            if (code == HttpStatus.SC_OK) {
                ...
                if (commands == null || versions == null) {
                    Log.e(K9MailLib.LOG_TAG, "OPTIONS response without commands or versions; reporting I/O error");
                ...
                Log.e(K9MailLib.LOG_TAG, "Validate: try folder sync");
                ...
                if (code == HttpStatus.SC_FORBIDDEN || code == HTTP_NEED_PROVISIONING) {
                    ...
                    Log.e(K9MailLib.LOG_TAG, "Validate: provisioning required");
                    if (svc.canProvision() != null) {
                        // If so, send the advisory Exception (the account may be created later)
                        Log.e(K9MailLib.LOG_TAG, "Validate: provisioning is possible");
                        //throw new MessagingException("MessagingException.SECURITY_POLICIES_REQUIRED");
                        return;
                    } else
                        Log.e(K9MailLib.LOG_TAG, "Validate: provisioning not possible");
                    ...
                    Log.e(K9MailLib.LOG_TAG, "Wrong address or bad protocol version");
                    ...
                    Log.e(K9MailLib.LOG_TAG, "Unexpected response for FolderSync: " + code);
                    throw new MessagingException("MessagingException.UNSPECIFIED_EXCEPTION");
                }
                Log.e(K9MailLib.LOG_TAG, "Validation successful");
                return;
            }
            if (isAuthError(code)) {
                Log.e(K9MailLib.LOG_TAG, "Authentication failed");
                ...
                Log.e(K9MailLib.LOG_TAG, "Validation failed, reporting I/O error: " + code);
                throw new MessagingException("MessagingException.IOERROR");
            }
        } catch (IOException e) {
            Throwable cause = e.getCause();
            if (cause != null && cause instanceof CertificateException) {
                Log.e(K9MailLib.LOG_TAG, "CertificateException caught: ", e);
                throw new MessagingException("MessagingException.GENERAL_SECURITY");
            }
            Log.e(K9MailLib.LOG_TAG, "IOException caught: ", e);
            throw new MessagingException("MessagingException.IOERROR");
        }
    }
    ...
    private static void setupProtocolVersion(EasStore service, Header versionHeader)
        ...
        Log.i(K9MailLib.LOG_TAG, "Server supports versions: " + supportedVersions);
        ...
        if (ourVersion == null) {
            Log.w(K9MailLib.LOG_TAG, "No supported EAS versions: " + supportedVersions);
            throw new MessagingException("MessagingException.PROTOCOL_VERSION_UNSUPPORTED");
        } else {
        ...
        synchronized (mInitializationLock) {
            if (mProtocolVersionDouble == null) {
                try {
                    ...
                    Log.e(K9MailLib.LOG_TAG, "Exception encountered while initializing EasStore", e);
                }
            }
    ...
    protected HttpResponse executePostWithTimeout(HttpClient client, HttpPost method, int timeout, boolean noProvision)
        ...
        if (isProvisionError(response.getStatusLine().getStatusCode())
            ...
            try {
                ...
                Log.e(K9MailLib.LOG_TAG, "Exception encountered while provisioning in EasStore", e);
            }
        }
        return response;
    }
    ...
    private void setHeaders(HttpRequestBase method, boolean usePolicyKey) {
        ...
        if (usePolicyKey) {
            ...
            if (mStoreConfig != null) {
                String accountKey = mStoreConfig.getSecurityKey();
    ...
    private void init() throws IOException, MessagingException {
        synchronized (mInitializationLock) {
            ...
            if (mProtocolVersion == null || lastSyncTimeDayDue) {
                Log.d(K9MailLib.LOG_TAG, "Determine EAS protocol version");
                HttpResponse resp = sendHttpClientOptions();
                reclaimConnection(resp);
                int code = resp.getStatusLine().getStatusCode();
                Log.d(K9MailLib.LOG_TAG, "OPTIONS response: " + code);
                if (code == HttpStatus.SC_OK) {
                    Header header = resp.getFirstHeader("MS-ASProtocolCommands");
                    Log.d(K9MailLib.LOG_TAG, header.getValue());
                    ...
                    Log.e(K9MailLib.LOG_TAG, "OPTIONS command failed; throwing IOException");
                    throw new IOException();
                }
            }
        }
    ...
    private List <? extends Folder > getInitialFolderList() throws MessagingException {
        ...
        try {
            ...
            try {
                ...
                if (code == HttpStatus.SC_OK) {
                    ...
                    Log.w(K9MailLib.LOG_TAG, "FolderSync response error: " + code);
                    throw new RuntimeException();
                }
            } finally {
                reclaimConnection(resp);
        ...
        //TODO: mAccount.saveFolderNames(Preferences.getPreferences(K9.app.getApplicationContext()));
    ...
    private boolean tryProvision() throws IOException, MessagingException {
        ...
        if (pp != null) {
            ...
            mStoreConfig.setSecurityKey(policyKey);
            //TODO: mStoreConfig.saveSecurityKey(Preferences.getPreferences(K9.app.getApplicationContext()));
            return true;
        } else {
            return false;
        }
    ...
    private void syncFoldersFromLocalStore() {
        try {
            ...
            Log.e(K9MailLib.LOG_TAG, "Exception encountered while restoring folders from the local store", e);
        }
    }
    ...
    public Folder getFolder(String serverId) {
        if (getStoreSyncKey().equals(INITIAL_SYNC_KEY)) {
            try {
                ...
                Log.e(K9MailLib.LOG_TAG, "Exception encountered while fetching the initial folder list", e);
            }
        }
        synchronized (mFolderList) {
            if (mFolderList.isEmpty()) {
...
//                }
                Log.d(K9MailLib.LOG_TAG, "Send cmd: " + cmd);

                HttpResponse resp = sendHttpClientPost(cmd, bodyEntity, SEND_MAIL_TIMEOUT);
                reclaimConnection(resp);
                int code = resp.getStatusLine().getStatusCode();
                if (code == HttpStatus.SC_OK) {
                    Log.d(K9MailLib.LOG_TAG, "Message sent successfully");
                } else {
                    Log.e(K9MailLib.LOG_TAG, "Message sending failed, code: " + code);
                }
            } catch (IOException e) {
                Log.e(K9MailLib.LOG_TAG, "Send failed: " + message.getUid());
            }
        }
    }
