 ...
 *
 */
package com.whiuk.philip.org.apache.http.impl.conn;
...
import com.whiuk.philip.org.apache.http.conn.ConnectionPoolTimeoutException;
import com.whiuk.philip.org.apache.http.conn.ConnectionRequest;
import com.whiuk.philip.org.apache.http.conn.DnsResolver;
import com.whiuk.philip.org.apache.http.conn.HttpClientConnectionManager;
import com.whiuk.philip.org.apache.http.conn.HttpClientConnectionOperator;
...
import com.whiuk.philip.org.apache.http.pool.ConnFactory;
import com.whiuk.philip.org.apache.http.pool.ConnPoolControl;
import com.whiuk.philip.org.apache.http.pool.PoolStats;
...
public class PoolingHttpClientConnectionManager
    ...
    public ConnectionRequest requestConnection(
        ...
        Args.notNull(route, "HTTP route");
            Timber.d("Connection request: " + format(route, state) + formatStats(route));
    ...
    protected HttpClientConnection leaseConnection(
        ...
        try {
            ...
            Asserts.check(entry.getConnection() != null, "Pool entry with no connection");
                Timber.d("Connection leased: " + format(entry) + formatStats(entry.getRoute()));
            ...
            try {
                if (conn.isOpen()) {
                    ...
                    entry.updateExpiry(keepalive, effectiveUnit);
                        ...
                        Timber.d("Connection " + format(entry) + " can be kept alive " + s);
                }
            } finally {
                this.pool.release(entry, conn.isOpen() && entry.isRouteComplete());
                    Timber.d("Connection released: " + format(entry) + formatStats(entry.getRoute()));
            }
        }
    }
    ...
    public void shutdown() {
        if (this.isShutDown.compareAndSet(false, true)) {
            Timber.d("Connection manager is shutting down");
            try {
                this.pool.shutdown();
            } catch (final IOException ex) {
                Timber.d(ex,"I/O exception shutting down connection manager");
            }
            Timber.d("Connection manager shut down");
        }
    }

    @Override
    public void closeIdleConnections(final long idleTimeout, final TimeUnit tunit) {
        Timber.d("Closing connections idle longer than " + idleTimeout + " " + tunit);
        this.pool.closeIdle(idleTimeout, tunit);
    }

    @Override
    public void closeExpiredConnections() {
        Timber.d("Closing expired connections");
