...
import com.whiuk.philip.org.apache.http.annotation.GuardedBy;
...
import com.whiuk.philip.org.apache.http.util.LangUtils;
...
public class BasicHttpClientConnectionManager implements HttpClientConnectionManager, Closeable {
    ...
    private void closeConnection() {
        if (this.conn != null) {
            ...
            try {
                ...
                Timber.d(iox,"I/O exception closing connection");
            }
            this.conn = null;
        }
    }
    ...
    private void shutdownConnection() {
        if (this.conn != null) {
            Timber.d("Shutting down connection");
            try {
                this.conn.shutdown();
            } catch (final IOException iox) {
                Timber.d(iox,"I/O exception shutting down connection");
            }
            this.conn = null;
        }
    }
    ...
    private void checkExpiry() {
        if (this.conn != null && System.currentTimeMillis() >= this.expiry) {
                Timber.d("Connection expired @ " + new Date(this.expiry));
            closeConnection();
        }
    }
    ...
    synchronized HttpClientConnection getConnection(final HttpRoute route, final Object state) {
        Asserts.check(!this.isShutdown.get(), "Connection manager has been shut down");
            Timber.d("Get connection for route " + route);
    ...
    public synchronized void releaseConnection(
        ...
        Asserts.check(conn == this.conn, "Connection not obtained from this manager");
            Timber.d("Releasing connection %s", conn);
